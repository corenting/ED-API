...
from models.database import System, Station, StationShipLink, Ship, CommodityPrice, Commodity
...
def get_system_stations(system_name):
    reference_system = db.session.query(System).filter(System.name == system_name).first()
    if reference_system is None:
        return error_response(system_name + ' system not found')

    db_stations = db.session.query(Station).filter(Station.system_id == reference_system.id).order_by(
        Station.distance_to_star).all()
    return db_stations
...
def get_system_info(system_name):
    ...
    sys_url = 'https://www.edsm.net/api-v1/systems?showInformation=1&showPermit=1&showCoordinates=1&systemName=' + \
              quote(system_name)
    ...
    if factions_infos is not None \
            and 'controllingFaction' in factions_infos and factions_infos['controllingFaction'] is not None:
    ...
    if factions_infos is not None:
        for item in factions_infos['factions']:
            factions_res.append({
                'name': item['name'],
                'updated_at': arrow.Arrow.utcfromtimestamp(item['lastUpdate']).isoformat(),
...
def get_station_details(system_name, station_name):
    ...
    db_station = db.session.query(Station).filter(
        and_(Station.name == station_name, Station.system_id == reference_system.id)).first()
    if db_station is None:
        return error_response(station_name + ' station not found')

    db_prices = db.session.query(CommodityPrice).filter(CommodityPrice.station_id == db_station.id).all()
    return {
        "station": db_station,
        "market": db_prices,
    }
...
def get_stations_selling_buying_commodity(reference_system_name, commodity, min_stock, min_demand, pad_size, selling):
    # First, try to get ref system from local db
    reference_system = db.session.query(System).filter(System.name == reference_system_name).first()
    ...
    for system in systems:
        for station in system.stations:
            if can_dock_at_station(station.max_landing_pad, pad_size):
                if not selling:
                    price = db.session.query(CommodityPrice).filter(and_(
                ...
                if price:
                    if (selling and price.demand >= min_demand) or ((not selling) and price.supply >= min_stock):
                        res.append({
                            "station": station,
                            "supply": price.supply,
                            "demand": price.demand,
                            ...
                            "distance": distance_between_systems(reference_system, station.system),
                            "last_price_update": arrow.Arrow.utcfromtimestamp(price.collected_at).isoformat(),
                            "distance_to_star": station.distance_to_star,
                            "price_difference_percentage":
                                price_difference(db_commodity.average_price,
                                                 price.buy_price if not selling else price.sell_price, selling),
                        })
...
def get_stations_selling_ship(reference_system_name, ship):
    ...
    stations = db.session.query(StationShipLink).filter(
        StationShipLink.ship_id == ship_id and
        reference_system.x - 25 <= StationShipLink.station.system.x <= reference_system.x + 25 and
        reference_system.y - 25 <= StationShipLink.station.system.y <= reference_system.y + 25 and
        reference_system.z - 25 <= StationShipLink.station.system.z <= reference_system.z + 25
    ).all()
    ret_list = [
        {
            'station': s.station,
