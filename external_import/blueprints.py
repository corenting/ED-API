...
from models.database import BlueprintEngineerLink, BlueprintIngredientLink, EngineerBlueprint, Engineer, \
    BlueprintIngredient
...
def import_blueprints(db_session):
    try:
        ...
        # Download JSON
        req = requests.get('https://raw.githubusercontent.com/msarilar/EDEngineer/master/EDEngineer/Resources/Data'
                           '/blueprints.json',
                           headers=get_requests_headers())

        if req.status_code != 200:
            raise ImportException('Error ' + str(req.status_code) + ' while downloading blueprints.json')
        ...
        for item in j:
            ...
            for eng in engineers_array:
                ...
                if not any(db_engineer.name == engineer.name for db_engineer in engineers_list):
            ...
            for ingredient in ingredients_array:
                ...
                if not any(db_ingredient.name == ingredient.name for db_ingredient in ingredients_list):
        ...
        for item in j:
            ...
            new_blueprint = EngineerBlueprint(name=item["Name"], type=item["Type"], grade=grade)
            ingredients_array = item["Ingredients"]
            engineers_array = item["Engineers"]
            for ingredient in ingredients_array:
                db_ingredient = next((x for x in ingredients_list if x.name == ingredient["Name"]), None)
                ingredient_link = BlueprintIngredientLink(blueprint=new_blueprint, ingredient=db_ingredient,
                                                          quantity=ingredient["Size"])
                db_session.add(ingredient_link)
            for engineer in engineers_array:
                db_engineer = next((x for x in engineers_list if x.name == engineer), None)
                engineer_link = BlueprintEngineerLink(blueprint=new_blueprint, engineer=db_engineer)
