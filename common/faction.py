...
def get_faction_history_item(system_name, faction):
    ...
    for attribute, value in faction['influenceHistory'].items():
        ...
        if type(faction['stateHistory']) != list:
            state_history_value = next((y for x, y in faction['stateHistory'].items() if x == attribute), None)
        else:
        ...
        if type(faction['pendingStatesHistory']) != list:
            pending_state_history_value = next(
                (y for x, y in faction['pendingStatesHistory'].items() if x == attribute), None)
        else:
        ...
        if type(faction['recoveringStatesHistory']) != list:
            recovering_state_history_value = next(
                (y for x, y in faction['recoveringStatesHistory'].items() if x == attribute), None)
        else:
    ...
    ret_items.append({
        ...
        'updated_at': arrow.Arrow.utcfromtimestamp(faction['lastUpdate']).isoformat(),
