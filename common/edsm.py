...
def get_system_from_edsm(name):
    try:
        # Download JSON
        url = 'https://www.edsm.net/api-v1/systems?showPermit=1&showId=1&showCoordinates=1&systemName=' + quote(name)
        req = requests.get(url, headers=get_requests_headers())
        ...
        req.close()
        return System(id=j["id"], x=j["coords"]["x"], y=j["coords"]["y"], z=j["coords"]["z"], name=j["name"],
